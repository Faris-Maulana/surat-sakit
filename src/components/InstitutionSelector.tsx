...
export default function InstitutionSelector({
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {institutionTypes.map((t) => (
            ...
            >
              <div className="w-10 h-10 flex items-center justify-center text-2xl bg-gray-50 rounded-lg">
                {t.icon}
              </div>
      ...
      {selectedType && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Pilih Kota</label>
          <select
            value={selectedCity}
              ...
              {filtered.map((inst) => (
                ...
                >
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: getLogoByType(inst.type) }}
                  />
                  ...
                  <ChevronRight className={`w-5 h-5 mt-2 ${selectedInstitution?.id === inst.id ? 'text-blue-500' : 'text-gray-300'}`} />
                </button>
              ))}
            </div>
          )}
